...
| `cfg.wx_depth_cols` | `{'SnoDAR_distance_Avg', 'SnoDAR_snow_depth_Avg'}` | snow-depth columns `{distance, snow_depth}` — **both required**: either header missing makes `load_snodar` return an empty table, so every weather overlay (depth, SWE, temperatures) becomes unavailable. The distance column feeds the drift/config-anomaly check. Set per site in `site_config.json`'s weather block |
