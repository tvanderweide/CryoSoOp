...
### Time-domain RFI gating (CSSL pulsed RFI)

Configured in the `"gating"` block of `site_config.json` (`toggle`, `mode`, `window_ms`,
`transition_ms` — unprefixed, the block namespaces them). `BrundageSoOp.m` copies them to the
`cfg` fields below and validates `mode`. **When the block is absent or `"toggle"` is false, none
of the `cfg.gating_*` fields are set** and `compute_L1`, `compute_rfi_spectrum`, and the viewer's
raw views all run ungated.

| Field | Entry value | Notes |
|---|---|---|
| `cfg.gating_toggle` | `true` only when `"gating"."toggle"` is true; otherwise UNSET | the single predicate every consumer tests with `isfield(cfg, 'gating_toggle')` |
| `cfg.gating_mode` | `'quiet'` | `'quiet'` keeps the MUOS intervals, `'loud'` keeps the RFI intervals. Any other value is a hard error (`BrundageSoOp:gatingMode`) at config load. There is no `'none'` mode — use `"toggle": false` |
| `cfg.gating_window_ms` | `0.5` ms | rolling mean-power window; the threshold is the midpoint of that window's P10/P90 |
| `cfg.gating_transition_ms` | `2.0` ms | blanked at BOTH capture ends and on EACH side of every internal transition. A capture with one transition loses `4 x transition_ms`; a capture with none still loses `2 x transition_ms` |
| `cfg.rfi_min_segments` | `16` | `compute_rfi_spectrum` warns (`BrundageSoOp:rfiLowSegmentCount`) below this season segment total |

Masked samples are **zeroed, not removed**: capture length is preserved, and the absolute power
columns (`pow_ch0_fd`, `pow_ch1_fd` and their `_muos` variants) scale with the surviving duty
cycle. `compute_rfi_spectrum` instead discards any segment that is not 100% active, and reads
`128*rfi_seg_len` samples per file (rather than `16*`) while gating is on — roughly an 8x rise in
peak per-worker memory, since two complex-double channel buffers of that length are live per
`parfor` worker. Override with `cfg.rfi_read_samples`.

**Enablement conditions — read before setting `"toggle": true` for science products:**

- Gated L1 power columns are not duty-cycle corrected, but the viewer's `Raw: PSD` view *is*
  (it rescales by the nonzero sample fraction). Viewer PSD levels and L1 power columns are
  therefore not directly comparable while gating is on. `compute_sigma0` consumes those power
  columns for direct-referenced calibration: ratios cancel the duty factor, absolute uses do
  not. **Do not enable gating for production sigma0, or interpret absolute gated powers, until
  this normalization is resolved.**
- The threshold is always the P10/P90 midpoint, so it assumes bimodal power. On data without
  pulsed RFI it splits a flat distribution at its own midpoint and rejects most of the capture.
  Enable only at sites with the CSSL-style periodic pulses.
- With `transition_ms = 2.0` and short loud bursts, the guard bands can consume every loud run,
  yielding an all-false mask. **Do not use `mode = 'loud'` without a preflight check of the
  quiet/loud/dropped sample fractions**; reduce `transition_ms` when working in that mode.
...
| `cfg.wx_temp_cols` | `{'AirTC_Avg', 'Temp_C_Avg'}` | temperature columns overlaid on the SNOdar plots (two toggleable lines, or — with the viewer's AboveFreezing box — one orange band layer over the times any ticked sensor is > 0 °C): `AirTC_Avg` = air temp (the melt-freeze driver); `Temp_C_Avg` = the sensor that `Processing/TemperaturePlot_SnowDepth.py` plots. Order maps to the viewer's `airtc_c`, `temp_c` toggles; the names also label the two temperature checkboxes and legend entries (truncated at 12 chars for the row budget). Set per site in `site_config.json`'s weather block |
| `cfg.wx_depth_cols` | `{'SnoDAR_distance_Avg', 'SnoDAR_snow_depth_Avg'}` | snow-depth columns `{distance, snow_depth}`. Both are optional, like every other sensor `load_snodar` reads: a missing header NaN-fills only its own column and warns, so the remaining weather overlays (SWE, temperatures, DTC, soil moisture) still load. A missing `snow_depth` header leaves `depth_m` all-NaN (the depth overlay and the snow-height σ⁰ variant go unavailable); a missing `distance` header only disables the drift/config-anomaly check, leaving the depth series unflagged. Set per site in `site_config.json`'s weather block |
| `cfg.wx_swe_cols` | `{'SS_SWE_Avg', 'SS_SWE_ErrCode_Avg'}` | snow-scale SWE columns `{value, errcode}` (mm water column) feeding the viewer's SWE overlay — drawn as a teal line in mm (the papers' SWE unit) on its own independent right ruler (label `SWE [mm]`): SWE owns the main right axis when snow depth is off, and moves to its own overlay ruler beside it when both are shown; depth always stays in meters. The viewer's `theoretical` box converts this SWE record to differential phase via the coherent-reflection paper's Eq. (6) fringe rate `606 mm × (260 MHz / f) × cos(θ_inc)/cos(43°)` (its `swe_per_fringe_mm` helper; ~365 mm/fringe at Brundage), anchored to a measured-phase reference, wrapped to [−180°, 180°), paper-positive sign. Reference: R. Shah et al., "Remote Sensing of Snow Water Equivalent Using P-Band Coherent Reflection," *IEEE GRSL* 14(3):309–313, 2017, doi:10.1109/LGRS.2016.2636664, Eq. (6). QC in `load_snodar`: with the errcode column present, values are kept only where the errcode is finite and exactly 0 (fail closed — the scale reads ≈0 while erroring, and a nonzero 15-min *average* errcode means part of the window errored); a value column without its errcode column keeps values unmasked with one partial-schema warning; a missing value column yields an all-NaN `swe_mm` silently (optional sensor). Then the depth spike idiom: \|SWE − 97-pt sliding median\| > 100 mm → NaN, and samples with < 13 finite window neighbors (~3 h at 15 min) are dropped as unverifiable. Set per site in `site_config.json`'s weather block; a site whose scale has a value column but no error-code column should still configure both names — the absent errcode header then takes the warn-and-keep-unmasked path |
| `cfg.wx_dtc_cols` | `'DTC_Avg(%d)'` | Digital Temperature Cable header pattern (or an explicit ordered list). A pattern expands from sensor 1 until a header is absent, so cables with different sensor counts need no per-site count setting (Brundage resolves to `DTC_Avg(1)`–`DTC_Avg(37)`). These columns are degrees Celsius and feed the `SnowTemp` thermograph on the four L2 Candidates views; missing columns remain NaN, so the other weather overlays still load. |
| `cfg.wx_dtc_sensor_spacing_cm` | `10.16` | DTC sensor separation in cm (4 in cable specification). Constant across sites; the sensor count is what varies. |
| `cfg.wx_dtc_bottom_height_cm` | `0` | Height above ground of the bottom DTC sensor in cm, **negative when the lowest sensors are buried in the soil**. The plot assumes the cable hangs straight down. Brundage is `-25.4`: three sensors (35–37) sit below ground, with the ground surface falling between sensors 34 and 35, so heights run −25.4 cm at the bottom to +340.4 cm at sensor 1. This is a site-calibratable reference value — `0` is the neutral default for a site whose burial depth has not been established. |
| `cfg.wx_dtc_order` | `'top_to_bottom'` | Physical order of `wx_dtc_cols`: the Brundage logger's `DTC_Avg(1)` is cable top (airward) and the highest-numbered sensor is cable bottom (groundward). Allowed values: `'top_to_bottom'`, `'bottom_to_top'`. |
| `cfg.wx_soil_vwc_cols` | `'SoilVUE_VWC_%dcm_Avg'` | SoilVUE volumetric-water-content header pattern (or an explicit ordered list), m³/m³, feeding the optional soil-moisture overlay on the `SnowTemp` subplot of the four L2 Candidates views. Unlike `wx_dtc_cols`, the pattern is substituted with the explicit `wx_soil_rod_cm` list rather than expanded sequentially, because rod positions are non-uniform. Every configured slot keeps its position: a header the logger does not carry stays an all-NaN column instead of shifting its neighbors. Fail-closed — a pattern carrying no `%d`-style conversion spec, or one where two rods expand to the same header, resolves to no columns at all rather than a partial list, since a dropped slot would shift the remaining columns out of step with `wx_soil_rod_cm` and mislabel every rod after the gap. Unset/empty ⇒ no soil overlay (rod geometry is site-specific and never guessed). |
| `cfg.wx_soil_rod_cm` | `[60, 75, 100]` | **Rod positions in cm — the numbers the logger headers carry, NOT depth below ground.** The Brundage probe is installed deliberately with its upper section above the surface, so displayed depth below ground is rod position − `wx_soil_surface_rod_cm`. List only rod segments actually buried in soil: segments in air or snow sit below the sensor's calibrated permittivity range (Campbell specs the SoilVUE 10 to ±1 permittivity unit over ε 4–42, while air/snow is ε ≈ 1.0–1.9), so their VWC calibration correctly refuses to extrapolate and returns exactly 0.000 m³/m³ year-round — not a sensor failure and not missing data. Brundage's rod-50 cm segment straddles the interface and is unusable as either snow or soil, so it is excluded too. Validated fail-closed: the list must be real, finite, unique, and **strictly increasing**, and every rod must sit strictly below `wx_soil_surface_rod_cm` — the overlay is disabled otherwise. Strictly increasing matters because the line color and style ramps key off configured order, so a descending list would contradict the shading the legend implies. |
| `cfg.wx_soil_surface_rod_cm` | `55` | Rod position of the soil surface in cm, on the same rod scale as `wx_soil_rod_cm`. Sets the zero of the displayed depth-below-ground labels (Brundage rods 60/75/100 cm ⇒ ~5/20/45 cm below ground). Site-measured at installation, and only approximately — which is why the legend labels read `~5 cm` rather than `5 cm`. **Required** whenever `wx_soil_rod_cm` is set: there is no default. Defaulting it to 0 would silently relabel rod positions as depths (rod 60 cm reading as "60 cm below ground"), so a missing or invalid value disables the overlay instead. |
...
| `cfg.snr_threshold` | `10` dB | used by compare_sat_candidates scoring; also seeds the candidates-family (L2: Sensor data + MUOS views) side-panel SNR-cutoff spinner (display-only floor applied with the same finite-SNR predicate before the daily-nearest pick; the CSV stores no threshold provenance, so this is the configured starting value, not a guarantee about older products) |
