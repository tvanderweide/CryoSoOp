...
| `cfg.wx_soil_vwc_cols` | `'SoilVUE_VWC_%dcm_Avg'` | SoilVUE volumetric-water-content header pattern (or an explicit ordered list), m³/m³, feeding the optional soil-moisture overlay on the `SnowTemp` subplot of the four L2 Candidates views. Unlike `wx_dtc_cols`, the pattern is substituted with the explicit `wx_soil_rod_cm` list rather than expanded sequentially, because rod positions are non-uniform. Every configured slot keeps its position: a header the logger does not carry stays an all-NaN column instead of shifting its neighbors. Unset/empty ⇒ no soil overlay (rod geometry is site-specific and never guessed). |
| `cfg.wx_soil_rod_cm` | `[60, 75, 100]` | **Rod positions in cm — the numbers the logger headers carry, NOT depth below ground.** The Brundage probe is installed deliberately with its upper section above the surface, so displayed depth below ground is rod position − `wx_soil_surface_rod_cm`. List only rod segments actually buried in soil: segments in air or snow sit below the sensor's calibrated permittivity range (Campbell specs the SoilVUE 10 to ±1 permittivity unit over ε 4–42, while air/snow is ε ≈ 1.0–1.9), so their VWC calibration correctly refuses to extrapolate and returns exactly 0.000 m³/m³ year-round — not a sensor failure and not missing data. Brundage's rod-50 cm segment straddles the interface and is unusable as either snow or soil, so it is excluded too. |
| `cfg.wx_soil_surface_rod_cm` | `55` | Rod position of the soil surface in cm, on the same rod scale as `wx_soil_rod_cm`. Sets the zero of the displayed depth-below-ground labels (Brundage rods 60/75/100 cm ⇒ ~5/20/45 cm below ground). Site-measured at installation. |
