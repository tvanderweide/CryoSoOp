...
function test_style_layout_and_gating(tc)
    ...
    verifyEqual(tc, V.sp_abvfrz_nearest_linew.Value, ...
                SoopViewerState.ABOVE_FREEZING_NEAREST_LINE_WIDTH);
    verifyEqual(tc, V.sp_linew.Limits, [0.25 5]);
    verifyEqual(tc, V.sp_ptsz.Limits, [0.25 5]);
    verifyEqual(tc, V.sp_abvfrz_nearest_linew.Limits, [1 20]);
    verifyEqual(tc, V.sp_linew.Step, 0.25);
    verifyEqual(tc, V.sp_ptsz.Step, 0.25);
    verifyEqual(tc, V.sp_abvfrz_nearest_linew.Step, 0.5);

    % Placement: directly under the legend row; grid has 23 x 28 px control
    ...
    verifyNumElements(tc, g.RowHeight, 33);
    verifyEqual(tc, [g.RowHeight{1:23}], repmat(28, 1, 23));
    verifyEqual(tc, [g.RowHeight{24:25}], [56 56]);
    ...
    verifyEqual(tc, V.abvfrz_nearest_width_row.Layout.Row, ...
                V.abvfrz_nearest_row.Layout.Row + 1);
    verifyFalse(tc, logical(V.abvfrz_nearest_row.Visible));
    verifyFalse(tc, logical(V.abvfrz_nearest_width_row.Visible));
    ...
    verifyTrue(tc, logical(V.abvfrz_nearest_width_row.Visible));
    verifyFalse(tc, logical(V.cb_abvfrz_nearest.Enable));
    V.cb_tod.Value = true;
