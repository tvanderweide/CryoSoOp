...
%
    ...
    % apply_gating drives time-domain gating explicitly rather than by dataset
    % name: NL/L are load states with no MUOS signal and are never gated.
    ds = struct( ...
        'name',         {'Signal',          'NL',                 'L'}, ...
        'pat',          {'UHF_2*_ch0.dat',  'UHF__NL_2*_ch0.dat', 'UHF__L_2*_ch0.dat'}, ...
        'sfx',          {'',                '_NL',                '_L'}, ...
        'use_sk',       {bp.use_sk,         false,                false}, ...
        'apply_gating', {true,              false,                false});
    for di = 1:numel(ds)
...
% filename pattern, output suffix, and band-finder SK gate differ.
    ...
    gate_cfg = struct(...
        'enable',        ap.gating_toggle && ds.apply_gating, ...
        'mode',          ap.gating_mode, ...
...
% surviving segments rather than all segments in the capture.
    ...
    for s = 1:nseg
        i1 = (s-1)*L + 1;  i2 = s*L;
        % Accumulate only fully-active segments; active_mask is the selected
        % quiet or loud mask, so a partially-gated segment contributes nothing.
