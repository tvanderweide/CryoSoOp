...
% Build the viewer uifigure and wire controls to callbacks that capture V.
    ...
    % Hour-of-day point coloring — shown on the satellite-candidates family
    % and on L2: Satellite elevation. Active only where the aggregation keeps
    % hour identity (Raw captures / Per-run mean), and, on the candidates
    % family only, while the daily filter is OFF; the render checks the same
    % predicate, so a checked-but-disabled box draws nothing.
    ...
    S.cb_hourcolor = uicheckbox(hour_row, 'Text', 'Color by hour', 'Value', false, ...
        'Tooltip', ['Color each displayed point by its nearest hour of day ' ...
